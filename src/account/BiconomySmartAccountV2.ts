...
export class BiconomySmartAccountV2 extends BaseSmartContractAccount {
  ...
  private constructor(
    ...
    const isDefaultFactory = [
      DEFAULT_BICONOMY_FACTORY_ADDRESS,
      TAIKO_FACTORY_ADDRESS,
    ].some((address) => addressEquals(defaultedFactoryAddress, address));
  ...
  private async getCounterFactualAddressV2(
    ...
    try {
      ...
      if (this.chainId === taikoHekla.id) {
        const factoryContract = getContract({
          ...
          client: {
            ...
            wallet: this.getSigner() as unknown as WalletClient,
          },
        });
      ...
      const initCalldata = encodeFunctionData({
        ...
        functionName: 'init',
        args: [this.defaultFallbackHandlerAddress, owner, moduleSetupData],
      });
  ...
  async sendTransaction(
    ...
    console.log(
      'lib defaultedBuildUseropDto?.params: ',
  ...
  public async getSessionParams({
    ...
    const defaultedConditionalSession: SessionSearchParam =
      store === 'DEFAULT_STORE'
        ? getDefaultStorageClient(accountAddress)
        : store ?? (await this.getAccountAddress());

    const defaultedCorrespondingIndexes: number[] | null = [
      'LAST_LEAF',
      'LAST_LEAVES',
    ].includes(String(leafIndex))
      ? null
      : leafIndex
      ? ((Array.isArray(leafIndex) ? leafIndex : [leafIndex]) as number[])
      : null;
  ...
  async getFactoryData() {
    ...
    const defaultValidationModuleAddress =
      this.defaultValidationModule.getAddress();
    const defaultValidationInitData =
